...
-- @check security-01 altinity-expert-clickhouse-security — safe, read-only evidence collection (fast first pass).
...
-- @check security-02 Full access dump (authoritative; redact secrets before sharing)
SHOW ACCESS;
...
-- @check security-03 User inventory: auth method, host ACL, default roles (references/02, 05, 13)
SELECT
    name, auth_type,
...
-- @check security-04 Roles and role graph (references/03)
SELECT name FROM system.roles ORDER BY name;
-- @check security-05 Role grants graph
...
-- @check security-06 Grant posture overview per principal (references/03)
SELECT
...
-- @check security-07 Admin-equivalent and access-management holders (references/03)
...
-- @check security-08 Security-sensitive privileges: external sources, code/secret/introspection, definer (references/03, 06, 12, 17)
SELECT user_name, role_name, access_type, grant_option
FROM system.grants
WHERE toString(access_type) IN (
...
-- @check security-09 Column-scoped SELECT grants vs sensitive-column exposure (references/08)
...
-- @check security-10 Row policies (references/08)
...
-- @check security-11 Settings profiles and the security-relevant elements (references/09)
SELECT name, storage, num_elements, apply_to_all, apply_to_list, apply_to_except
FROM system.settings_profiles
ORDER BY name;
-- @check security-12 Settings profile elements
...
-- @check security-13 Quotas (references/09)
...
-- @check security-14 Named collections — names only; secrets are masked, never select values (references/12)
...
-- @check security-15 User-defined / executable functions = code-execution surface (references/17)
...
-- @check security-16 confirm tcp_port_secure / https_port / listen_host from config.xml (references/05).
...
-- @check security-17 Cluster topology and the user used for distributed access (references/15, 16).
...
-- @check security-18 Persistent external-engine tables and any SQL SECURITY DEFINER/NONE views (references/04, 07)
...
-- @check security-19 Logging/auth-audit baseline (references/10, 11).
...
-- @check security-20 Table existence is the key signal: session_log_exists = 0 means login/auth auditing is OFF.
SELECT
